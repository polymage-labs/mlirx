...
//===----------------------------------------------------------------------===//
//
// This file implements a pass to normalize interprocedural memref layouts.
//
//===----------------------------------------------------------------------===//
...
// func @matmul(%A: memref<16xf64, #pad>, %B: index, %C: memref<16xf64>) ->
// (memref<16xf64, #pad>) {
...
void NormalizeMemRefs::runOnOperation() {
  ...
  module.walk([&](FuncOp funcOp) {
    ...
    SmallVector<Type, 8> inputs;  // Holds the arguments of function.
    SmallVector<Type, 4> results; // Holds the results of function.
    ...
    for (auto argIndex : llvm::seq<unsigned>(0, funcOp.getNumArguments())) {
      ...
      if (funcOp.getArgument(argIndex).getType().getKind() ==
          StandardTypes::MemRef) {

        MemRefType memrefType =
            funcOp.getArgument(argIndex).getType().cast<MemRefType>();
        ...
        auto oldMemRef = funcOp.getArgument(argIndex);

        // Insert a new temporary argument with the new memref type.
        funcOp.front().insertArgument(argIndex,
                                      static_cast<Type>(newMemRefType));
        ...
        // Since in this pass the objective is to normalize arguments of a
        // function and replace the uses accordingly, we check if the function
        // return type uses the same old memref type.
