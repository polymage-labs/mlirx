...
struct AffineScalarReplacement
  ...
  LogicalResult forwardStoreToLoad(FuncOp f);
...
LogicalResult AffineScalarReplacement::forwardStoreToLoad(FuncOp f) {
  ...
  return success(loadOpsToErase.size() > 0);
}
