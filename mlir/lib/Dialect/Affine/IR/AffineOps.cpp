...
struct AffineStrideNormalizer : public OpRewritePattern<AffineForOp> {
  ...
  LogicalResult matchAndRewrite(AffineForOp forOp,
    ...
    forOp.getInductionVar().replaceAllUsesExcept(ivScaleUp, exceptions);
...
void AffineForOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                              MLIRContext *context) {
  results.add<AffineForEmptyLoopFolder, AffineStrideNormalizer>(context);
}
...
verifyMemoryOpIndexing(AffineMemOpTy op, AffineMapAttr mapAttr,
  ...
  if (failed(verifyDimAndSymbolIdentifiers(op, op.getMapOperands(), numDims)))
    return failure();
...
static LogicalResult verify(AffineYieldOp op) {
  ...
  if (!isa<AffineParallelOp, AffineIfOp, AffineForOp, AffineExecuteRegionOp>(
          parentOp))
